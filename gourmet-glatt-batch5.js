...
setTimeout(()=>{
 ...
 dateTap=function(ds,ev){if(ev==='Shabbos Night'||ev==='Shabbos Day'){supMode='shab';nav('suppers');document.querySelectorAll('[data-sup]').forEach(b=>b.classList.toggle('on',b.dataset.sup===supMode));renderSuppers();return}if(ev){supMode='yt';nav('suppers');document.querySelectorAll('[data-sup]').forEach(b=>b.classList.toggle('on',b.dataset.sup===supMode));renderSuppers();return}openSupperPicker(ds)};
