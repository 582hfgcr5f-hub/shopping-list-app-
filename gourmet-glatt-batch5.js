/* Clean/modern preview. Catalogue, storage keys and stable feature files are unchanged. */
...
(() => {
  ...
  const release = window.ShoppingRelease = {
    version: 'clean-modern-preview-20260908-1',
  ...
  const assets = [
    ...
    {name: 'modern-theme', path: 'modern-interface.css', type: 'style', ready: () => true},
    {name: 'modern-interface', path: 'modern-interface.js', ready: () => !!window.ModernInterface?.ready}
  ];
  ...
  function loadScript(asset) {
    return new Promise((resolve, reject) => {
      ...
      if (asset.type === 'style') { node.rel = 'stylesheet'; node.href = url.href; }
      ...
      node.onload = () => finish();
      node.onerror = () => finish(new Error('Could not load ' + asset.name));
  ...
  function showFailure(error) {
    ...
    reload.type = 'button'; reload.textContent = 'Reload app'; reload.className = 'btn';
  ...
  window.ShoppingAppReady = domReady.then(async () => {
    ...
    if (typeof renderCats !== 'function' || typeof renderSuppers !== 'function' || typeof N !== 'function') throw new Error('The base app did not initialize');
    for (const asset of assets) {
      await loadScript(asset); await waitUntil(asset.ready, asset.name); release.completed.push(asset.name);
    }
    release.state = 'ready';
